...
def assemble_dataset(positive_source_dir, hard_negatives_dir, output_dir, val_split_ratio=0.2, random_seed=42):
    ...
    def copy_files(file_list, src_img_dir, src_lbl_dir, dest_img_dir, dest_lbl_dir):
        for base_name in file_list:
            if (src_img_dir / (base_name + '.jpg')).exists(): shutil.copy(src_img_dir / (base_name + '.jpg'), dest_img_dir)
    ...
    positive_files = [p.stem for p in pos_train_img_dir.glob("*.jpg")]; random.shuffle(positive_files)
    ...
    negative_files = [p.stem for p in neg_img_dir.glob("*.jpg")]; random.shuffle(negative_files)
    ...
    test_pos_files = [p.stem for p in test_pos_img_dir.glob("*.jpg")]
    
    # 獲取 test_neg 的所有檔案
    test_neg_lbl_dir = positive_source_dir / "labels" / "test_neg"
    test_neg_files = [p.stem for p in test_neg_img_dir.glob("*.jpg")]
...
if __name__ == '__main__':
    ...
    POSITIVE_SOURCE_DIRECTORY = "/home/yuan/OIL_PROJECT/dataset/dataset_SAR_2/SAR_2_PNG_relabel_rearrange_Patch/Patched_P1024_O128_BG100p_Separated/SAR_2_PNG_relabel_rearrange"

    # 2. 指定由 mine_hard_negatives.py 挖掘出的困難負樣本資料夾
    HARD_NEGATIVES_DIRECTORY = "/home/yuan/OIL_PROJECT/dataset/dataset_SAR_2/SAR_2_PNG_relabel_rearrange_Patch/HNM_Step2_Negatives_Mined_SAR2_P1024"

    # 3. 指定最終要輸出的黃金資料集的名稱與路徑
    OUTPUT_DIRECTORY = "/home/yuan/OIL_PROJECT/dataset/dataset_SAR_2/SAR_2_PNG_relabel_rearrange_Patch/HNM_Step3_Final_SAR_P1024"
