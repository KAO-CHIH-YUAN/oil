...
class RS3Mamba(nn.Module):
    def __init__(self,
        ...
        self.backbone = timm.create_model(backbone_name, features_only=True, output_stride=32,
                                          out_indices=(1, 2, 3, 4), pretrained=pretrained, in_chans=in_channels) # <--- [修改] 加入 in_chans
        ...
        # 修改 stem 層，使用 in_channels
        self.stem = nn.Sequential(
              nn.Conv2d(in_channels, 48, kernel_size=7, stride=2, padding=3), # <--- [修改] 3 改為 in_channels
              nn.InstanceNorm2d(48, eps=1e-5, affine=True),
        )
...
def load_pretrained_ckpt(
    ...
    for k, v in ckpt['model'].items():
        ...
        if kr in model_dict.keys():
            # [新增] 如果形狀不匹配 (例如第一層 conv)，則跳過
            if v.shape != model_dict[kr].shape:
                print(f"Shape mismatch for {kr}: checkpoint {v.shape} vs model {model_dict[kr].shape}. Skipping.")
                continue
