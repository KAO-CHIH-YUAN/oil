...
class SegformerAdapter(nn.Module):
        ...
        for epoch in range(epochs):
            ...
            with torch.no_grad():
                ...
                for i, (images, masks) in enumerate(val_iterator):
                    if not sys.stdout.isatty() and (i + 1) % val_log_interval == 0:
    ...
    def val(self, data, split='test', **kwargs):
        ...
        with torch.no_grad():
            if sys.stdout.isatty():
                test_iterator = tqdm(test_loader, desc="Calculating Test Metrics")
            else:
                test_iterator = test_loader
                
            for i, (images, masks) in enumerate(test_iterator):
                if not sys.stdout.isatty() and (i + 1) % 50 == 0:
